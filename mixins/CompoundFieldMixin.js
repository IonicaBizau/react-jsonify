...
module.exports = {
	renderHeader: function renderHeader() {
		...
		if (settingsHeader === false) return '';

		var type = typeof settingsHeader === 'undefined' ? 'undefined' : _typeof(settingsHeader),
		    header;
	...
	toggleEditing: function toggleEditing() {
		if (this.state.editing != 'always' && this.props.settings.header !== false) this.setState({ editing: !this.state.editing });
	},

	componentWillReceiveProps: function componentWillReceiveProps(nextProps) {
		if (this.props.settings.editing != nextProps.settings.editing) this.setState({ editing: nextProps.editing });
	},

	renderAdder: function renderAdder(name) {
		var settingsAdder = this.props.settings.adder,
		    type = typeof settingsAdder === 'undefined' ? 'undefined' : _typeof(settingsAdder),
		    adder;
	...
	createField: function createField(key, value, definition) {

		if (this.props.value[key]) return console.log('Field ' + key + ' already exists.');
  ...
  */
	checkEditingSetting: function checkEditingSetting(key) {
		var fields = this.state.fields;
		if (fields[key] && fields[key].settings.focus === true) {
			fields = assign({}, fields);
			var settings = fields[key].settings;
			settings.focus = false;
			if (settings.editing != 'always') settings.editing = false;
			this.setState({ fields: fields });
		}
	},

	getFixedFields: function getFixedFields() {
